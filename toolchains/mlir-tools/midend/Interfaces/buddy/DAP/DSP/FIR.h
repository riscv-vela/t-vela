...
#ifndef FRONTEND_INTERFACES_npu_DAP_DSP_FIR
#define FRONTEND_INTERFACES_npu_DAP_DSP_FIR

#include "npu/Core/Container.h"
#include "npu/DAP/AudioContainer.h"
#include "npu/DAP/DSP/Window.h"
...
void _mlir_ciface_npu_fir(MemRef<float, 1> *inputnpuFIR,
                            MemRef<float, 1> *kernelnpuFIR,
                            MemRef<float, 1> *outputnpuFIR);
void _mlir_ciface_npu_fir_vectorization(MemRef<float, 1> *inputnpuFIR,
                                          MemRef<float, 1> *kernelnpuFIR,
                                          MemRef<float, 1> *outputnpuFIR);
}
} // namespace detail
...
void FIR(MemRef<float, N> *input, MemRef<T, N> *filter,
  ...
  if (!isVectorization)
    detail::_mlir_ciface_npu_fir(input, filter, output);
  else
    detail::_mlir_ciface_npu_fir_vectorization(input, filter, output);
}
} // namespace dap

#endif // FRONTEND_INTERFACES_npu_DAP_DSP_FIR
