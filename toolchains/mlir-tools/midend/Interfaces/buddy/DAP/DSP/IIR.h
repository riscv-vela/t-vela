...
#ifndef FRONTEND_INTERFACES_npu_DAP_DSP_IIR
#define FRONTEND_INTERFACES_npu_DAP_DSP_IIR
...
void _mlir_ciface_npu_iir(MemRef<float, 1> *inputnpuConv1D,
                            MemRef<float, 2> *kernelnpuConv1D,
                            MemRef<float, 1> *outputnpuConv1D);

void _mlir_ciface_npu_iir_vectorization(MemRef<float, 1> *inputnpuConv1D,
                                          MemRef<float, 2> *kernelnpuConv1D,
                                          MemRef<float, 1> *outputnpuConv1D);
}
} // namespace detail
...
void IIR(MemRef<float, N> *input, MemRef<T, 2> *filter,
  ...
  if (!isVectorization)
    detail::_mlir_ciface_npu_iir(input, filter, output);
  else
    detail::_mlir_ciface_npu_iir_vectorization(input, filter, output);
}

} // namespace dap

#endif // FRONTEND_INTERFACES_npu_DAP_DSP_IIR
